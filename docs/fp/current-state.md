...
2. **时效检查 (按通路)**: 20s 阈值
   - Direct 通路: `dtu` (USDT/USD)
   - EUR 通路: `dte` (USDT/EUR) + `deu` (EUR/USD)
   - USDC 通路: `dct` (USDC/USDT) + `dcu` (USDC/USD)
   - 单通路 stale → warn + 自动降级为剩余通路
   - 三通路全 stale → ERROR + `AIM_EXCH_INVALID`
3. 每条有效路径独立计算 mid-price (BBO level)
4. **一致性检测** (`check_path_consistency`): 支持 1/2/3 条通路
   - 3 条: median 抗单路径偏离, >2bp 丢弃
   - 2 条: 互相比较, 均保留
   - 1 条: 自动通过
   - 全部偏离 → `AIM_EXCH_INVALID`
5. 通过检测的路径用 `append_path_levels` 构造 Level (price, qty)
   - Direct: 用 depth 前 10 档
   - EUR/USDC 路径: 双腿合成, depth_T=10, depth_E/C=5
6. `solve_equilibrium_price`: 在合成后的 bids/asks 上找 `|B(P)-A(P)|` 最小的价格
